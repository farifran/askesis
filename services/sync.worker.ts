...
// NEW PAYLOAD FOR QUOTE ANALYSIS
...
// NEW: Quote Analysis Prompt Builder
        ...
        if (modified) {
            if (Object.keys(yearData).length === 0) {
                ...
                try {
                    ...
                    console.warn(`Worker: Failed to compress pruned archive for year ${yearStr}. Falling back to JSON.`, e);
                    updatedArchives[yearStr] = JSON.stringify(yearData);
                }
            }
        }
    }
...
type WorkerRequest = 
    ...
    | { id: string; type: 'build-quote-analysis-prompt'; payload: QuoteAnalysisPayload } // NEW
    | { id: string; type: 'archive'; payload: ArchiveInputPayload }
    | { id: string; type: 'prune-habit'; payload: PruneInputPayload }; 
...
self.onmessage = async (e: MessageEvent<WorkerRequest>) => {
    ...
    try {
        ...
        if (type === 'encrypt') {
            ...
            result = await buildQuoteAnalysisPrompt(payload as QuoteAnalysisPayload);
