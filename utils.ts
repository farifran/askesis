...
export function generateUUID(): string {
    ...
    // ROBUSTNESS [2025-05-09]: Wrap in try-catch.
    // crypto.randomUUID can throw 'SecurityError: The operation is insecure' in non-secure contexts (http://IP).
    try {
    ...
    // Try crypto.getRandomValues (also might require secure context)
    try {
    ...
    if (!usedCrypto) {
        // Fallback inseguro (Math.random) apenas se crypto não existir ou falhar
        for (let i = 0; i < 16; i++) {
